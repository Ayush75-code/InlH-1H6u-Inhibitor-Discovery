...
# Create figure with transparent background and better aspect ratio
fig = plt.figure(figsize=(20, 10), facecolor='none')  # Transparent background

# Add main title
fig.suptitle('Principal Component Analysis (PCA) and Free Energy Landscape (FEL) Models', 
             fontsize=16, fontweight='normal', y=0.95)

# Define positions for subplots with better aspect ratios
plot_width = 0.18
plot_height = 0.35
pca_bottom = 0.55
fel_bottom = 0.1
...
for idx, (system_name, (xvg_file, _)) in enumerate(systems.items()):
    ax = fig.add_axes(pca_positions[idx], facecolor='none')  # Transparent subplot background
    
    if os.path.exists(xvg_file):
        pc1, pc2 = read_xvg_safe(xvg_file)
        
        if len(pc1) > 100:
            # Set equal aspect ratio for PCA plots
            ...
            # Add colorbar with transparent background
            cbar = plt.colorbar(h, ax=ax, pad=0.02, fraction=0.046)
            cbar.set_label('Count', fontsize=8, labelpad=6)
            cbar.ax.tick_params(labelsize=7, length=2)
            cbar.outline.set_linewidth(0.8)
            cbar.ax.set_facecolor('none')  # Transparent colorbar background
            
            print(f"✓ PCA: {system_name} ({len(pc1)} points)")
        else:
...
for idx, (system_name, (_, xpm_file)) in enumerate(systems.items()):
    ax = fig.add_axes(fel_positions[idx], facecolor='none')  # Transparent subplot background
    ...
    if os.path.exists(xpm_file):
        ...
        if prob_matrix is not None:
            ...
            if fel_matrix is not None:
                # Set correct aspect ratio for FEL plots (square pixels)
                ax.set_aspect('equal', adjustable='box')
...
plt.savefig('combined_pca_fel_publication_FIXED.png', dpi=300, bbox_inches='tight',
            facecolor='none', edgecolor='none', transparent=True)  # Transparent PNG
plt.savefig('combined_pca_fel_publication_FIXED.eps', format='eps', dpi=300, 
            bbox_inches='tight', facecolor='none', edgecolor='none', transparent=True)  # Transparent EPS
print("\n✓ Saved combined_pca_fel_publication_FIXED.png and .eps with transparent background")
plt.close()
